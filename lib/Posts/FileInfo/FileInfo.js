class FileInfo {
    ...
    static addMeta(file) {
        const meta = {
            ...
            thumbFullSrc: FileInfo.thumbFullSrc(file),
            thumbOriginalSrc: FileInfo.thumbOriginalSrc(file),
            fullSrc: FileInfo.fullSrc(file),
            originalSrc: FileInfo.fullSrc(file, true),
        };
    ...
    static isVideo(extension) {
        return FileInfo.getFileType(extension) === 'video';
    }
    ...
    static thumbFullSrc({ board, filename, extension }) {
        if (FileInfo.isVideo(extension))
            return FileInfo.thumbUrl({ board, filename });
        else
            return FileInfo.cacheUrl({ board, filename });
    }
    ...
    static fullSrc({ board, filename, extension }, original = false) {
        if (original)
