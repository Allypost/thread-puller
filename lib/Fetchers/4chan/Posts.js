...
import { callStorage, publishEvent } from '../../../lib/Redis/redis';
...
export default class Posts {
    ...
    static async get(board, thread) {
        ...
        return this.refreshCached(board, thread);
    }
    ...
    static async setCached(board, thread, posts) {
        ...
        if (posts.length) {
            await callStorage(
                ...
                'EX',
                String(Number(process.env.THREADPULLER_API_CACHE_FOR) / 2),
            );
        }
    ...
    static async refreshCached(board, thread) {
        const data = await this.setCached(board, thread, await this.getLive(board, thread));

        await publishEvent('page-data:update:posts', JSON.stringify({ board, thread, data }));
